...
-- deduplicate by getting first chronological view from a set of duplicates
          ...
          meta_url
        FROM
          `qubit-client-CLIENTPROJECTNUM.PROJECTNAME__v2.event_ecView`
        WHERE
  ...
  ,  product AS (SELECT
      ...
      FROM
        (SELECT
          ...
          meta_type
        FROM
          `qubit-client-CLIENTPROJECTNUM.PROJECTNAME__v2.event_ecBasketItemTransaction`
        WHERE
        ...
        SELECT
          ...
          meta_type
        FROM
          `qubit-client-CLIENTPROJECTNUM.PROJECTNAME__v2.event_ecProduct`
        WHERE
         TIMESTAMP_ADD(meta_serverTs, INTERVAL 3600000000 MICROSECOND) >= TIMESTAMP_TRUNC(TIMESTAMP_ADD(current_timestamp,INTERVAL 3600000000 MICROSECOND) ,DAY)
          --((( meta_serverTs ) >= ((TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY))) AND ( meta_serverTs ) < ((TIMESTAMP_ADD(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY), INTERVAL 1 DAY)))))
        UNION ALL
        SELECT
          ...
          meta_type
        FROM
          `qubit-client-CLIENTPROJECTNUM.PROJECTNAME__v2.event_ecBasketItem`
        WHERE
          TIMESTAMP_ADD(meta_serverTs, INTERVAL 3600000000 MICROSECOND) >= TIMESTAMP_TRUNC(TIMESTAMP_ADD(current_timestamp,INTERVAL 3600000000 MICROSECOND) ,DAY)
          --((( meta_serverTs ) >= ((TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY))) AND ( meta_serverTs ) < ((TIMESTAMP_ADD(TIMESTAMP_TRUNC(CURRENT_TIMESTAMP(), DAY), INTERVAL 1 DAY)))))
        )
